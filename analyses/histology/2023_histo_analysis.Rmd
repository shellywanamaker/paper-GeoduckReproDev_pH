...
#read in ind. egg size measurements
...
a <- quant_qual_long %>% filter(metric == "prop_egg") %>% filter(scorer == "Crandall") %>% ggplot(aes(x = as.numeric(qual_stage), y = um, group = interaction(sample_ID,scorer))) + geom_smooth(aes(group = Sex),method = "lm", color="black", fill = "gray80",size = 0.5)  + geom_point(aes(shape = pCO2),size = 2)+ stat_poly_eq(aes(group = Sex,label = paste( after_stat(adj.rr.label), "*\", \"*", after_stat(p.value.label),sep = "")), label.x = "left", label.y = "top",formula = y~x, parse = TRUE)+ labs(x = "stage", y = "eggs (% total area)", color =expression(paste(italic("p"),"CO", sep = "")[2]),shape =expression(paste(italic("p"),"CO", sep = "")[2])) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), axis.line = element_line(colour = "black")) + scale_y_continuous(limits =c(0,25)) + scale_x_continuous(n.breaks =3) +  scale_shape_manual(values = c(1,16)) + theme(legend.position = "none",text = element_text(size = 12))

#plot follicle area x stage
b <- quant_qual_long%>% filter(metric == "prop_no_tissue") %>% filter(scorer == "Crandall") %>% ggplot(aes(x = as.numeric(qual_stage), y = um, group = interaction(sample_ID,scorer))) + geom_smooth(aes(group = Sex),method = "lm", color="black", fill = "gray80", size = 0.5)+ geom_point(aes(shape = pCO2), size = 2)  + stat_poly_eq(aes(group = Sex,label = paste(after_stat(adj.rr.label), "*\", \"*", after_stat(p.value.label),sep = "")), label.x = "left", label.y = "top",formula = y~x, parse = TRUE)  + labs(x = "stage", y = "follicles (% total area)", color =expression(paste(italic("p"),"CO", sep = "")[2]),shape =expression(paste(italic("p"),"CO", sep = "")[2])) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), axis.line = element_line(colour = "black"))  + scale_shape_manual(values = c(1,16)) + scale_x_continuous(n.breaks =3)+ scale_y_continuous(limits =c(0,85))+theme(legend.position = "none",text = element_text(size = 12))


#plot conx tissue area x stage
c <- quant_qual_long %>% filter(metric == "prop_cnx_tissue_indve") %>% filter(scorer == "Crandall")%>% ggplot(aes(x = as.numeric(qual_stage), y = um, group = interaction(sample_ID,scorer)))  + geom_smooth(aes(group = Sex),method = "lm", color="black", fill = "gray80", size =0.5)+ geom_point(aes(shape = pCO2),size = 2) + stat_poly_eq(aes(group = Sex,label = paste(after_stat(adj.rr.label), "*\", \"*", after_stat(p.value.label),sep = "")), label.x = "left", label.y = "top",formula = y~x, parse = TRUE) + labs(x = "stage", y = "connective tissue (% total area)", color =expression(paste(italic("p"),"CO", sep = "")[2]),shape =expression(paste(italic("p"),"CO", sep = "")[2])) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), axis.line = element_line(colour = "black"))  + scale_shape_manual(values = c(1,16)) + scale_x_continuous(n.breaks =3)+scale_y_continuous(limits =c(0,80))+theme(legend.key=element_rect(fill="white"),legend.box.background = element_rect(colour = "black"),legend.box.margin = margin(t = 1, l = 1),legend.position = c(0.83, 0.85), text = element_text(size = 12))


d <- quant_qual_m_summ %>% ggplot(aes(x = as.numeric(Stage_Crandall), y = mean_perc_lumen)) + geom_smooth(aes(group = Sex),method = "lm", color="black", fill = "gray80",size = 0.5)  + geom_point(aes(shape = pCO2),size = 2)+ stat_poly_eq(aes(group = Sex,label = paste(after_stat(adj.rr.label), "*\", \"*", after_stat(p.value.label),sep = "")), label.x = "left", label.y = "top",formula = y~x, parse = TRUE)+ labs(x = "stage", y = "lumen (% area total acini)", color =expression(paste(italic("p"),"CO", sep = "")[2]),shape =expression(paste(italic("p"),"CO", sep = "")[2])) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(), axis.line = element_line(colour = "black"))  + scale_shape_manual(values = c(1,16)) +theme(legend.position = "none",text = element_text(size = 12))

e <- quant_qual_m_summ  %>% ggplot(aes(x = as.numeric(Stage_Crandall), y = mean_perc_zoa)) + geom_smooth(aes(group = Sex),method = "lm", color="black", fill = "gray80",size = 0.5)  + geom_point(aes(shape = pCO2),size = 2)+ stat_poly_eq(aes(group = Sex,label = paste(after_stat(adj.rr.label), "*\", \"*", after_stat(p.value.label),sep = "")), label.x = "left", label.y = "top",formula = y~x, parse = TRUE)+ labs(x = "stage", y = "spermatozoa (% area total acini)", color =expression(paste(italic("p"),"CO", sep = "")[2]),shape =expression(paste(italic("p"),"CO", sep = "")[2])) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(), axis.line = element_line(colour = "black"))  + scale_shape_manual(values = c(1,16)) +theme(legend.position = "none",text = element_text(size = 12))

f <- quant_qual_m_summ   %>% ggplot(aes(x = as.numeric(Stage_Crandall), y = mean_perc_cytes)) + geom_smooth(aes(group = Sex),method = "lm", color="black", fill = "gray80",size = 0.5)  + geom_point(aes(shape = pCO2),size = 2)+ stat_poly_eq(aes(group = Sex,label = paste(after_stat(adj.rr.label), "*\", \"*", after_stat(p.value.label),sep = "")), label.x = "right", label.y = "top",formula = y~x, parse = TRUE)+ labs(x = "stage", y = "spermatocytes (% area total acini)", color =expression(paste(italic("p"),"CO", sep = "")[2]),shape =expression(paste(italic("p"),"CO", sep = "")[2])) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(), axis.line = element_line(colour = "black"))  + scale_shape_manual(values = c(1,16)) +theme(legend.position = "none",text = element_text(size = 12))

ggarrange(a,b,c,d,e,f, ncol = 3,nrow = 2, labels = "auto", common.legend = F)
dev.off()
...
#plot all metrics for female quantitative data
jpeg("all_female_metrics_facets.jpg", width = 12, height = 6, units = "in", res = 300)
merge(female_data_STACKED, data.frame(cbind(metric = sort(unique(female_data_STACKED[!grepl("total.area..um.|sd.egg.size",female_data_STACKED$metric),"metric"])), metric2 = c("connective tissue area (um2)","egg area (um2)", "mean egg size (um2)","median egg size (um2)", "follicle area (um2)", "number of eggs", "number of eggs per um2", "proportion of connective tissue (%)", "proportion of egg (%)", "proportion of follicle (%)"))), by = "metric")%>% group_by(metric) %>% mutate(date = ifelse(date == "20190123", "72 d", "8 d post")) %>% mutate(pCO2 = ifelse(pH == "low", "elevated", "ambient")) %>% ggplot(aes(x = date, y = um, fill = pCO2)) + geom_bar(stat = "identity", position = "dodge")+ facet_wrap(~metric2, scale = "free", ncol = 5)+ theme_bw() + labs(x = NULL, y = NULL) + theme(legend.position = "top") + scale_fill_manual(values = c("#00BFC4","#F8766D"))
dev.off()

#make table for ANOVA results for female data
jpeg("female_ANOVA_table.jpg", width = 7, height = 4, units = "in", res = 300)
female_data_STACKED %>% mutate(time = ifelse(date == "20190123", "72 d", "8 d post"))%>% group_by(metric) %>% do(model = broom::tidy(aov(um ~ pH * time, data =.)) ) %>% unnest_wider(model)  %>% dplyr::select(c(metric, term,statistic, p.value)) %>% unnest(term, statistic, p.value) %>% pivot_longer(names_to = "term2", values_to = "value", cols = 3:4) %>% mutate(name = paste0(term," ",term2)) %>% filter(term != "Residuals") %>% dplyr::select(metric, name, value) %>% pivot_wider(names_from = name, values_from = value) %>% filter(metric != 'sd.egg.size') %>% filter(metric != 'total.area..um.') %>% mutate(metric = c("connective tissue area (um2)","egg area (um2)", "mean egg size (um2)","median egg size (um2)", "follicle area (um2)", "number of eggs", "number of eggs per um2", "proportion of connective tissue (%)", "proportion of egg (%)", "proportion of follicle (%)")) %>% kbl(caption = "ANOVA results: metric ~ pH * time") %>%  kable_classic(full_width = F, html_font = "Cambria")
dev.off()
```
