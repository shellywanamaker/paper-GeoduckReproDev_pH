...
male_quant_wd <- male_quant %>%dplyr::select(-c(Meas_num,Label,hue,saturation,brightness)) %>% pivot_wider(names_from=type, values_from=Area) %>% mutate(perc_zoa = zoa/total_area*100) %>% mutate(perc_cytes=(cytes_zoa - zoa)/total_area*100) %>% mutate(perc_lumen=lumen/total_area*100) %>% mutate(cytes = cytes_zoa - zoa)
...
quant_qual_m <- merge(male_quant_wd,staging, by = "sample_ID")
...
#create PCO2 column
quant_qual_m <- quant_qual_m %>% mutate(pCO2 = factor(ifelse(Treatment.x == "Low","elevated", "ambient"), levels = c("elevated","ambient"))) 

#calculate means for each animal
quant_qual_m_summ <- quant_qual_m %>% group_by(sample_ID, pCO2, Stage_Crandall,Sex) %>% summarise(mean_perc_zoa = mean(perc_zoa), sd_perc_zoa = sd(perc_zoa), mean_perc_cytes = mean(perc_cytes), sd_perc_cytes = sd(perc_cytes), mean_perc_lumen = mean(perc_lumen), sd_perc_lumen = sd(perc_lumen))


#reformat Female data
quant_qual_long <- quant_qual %>% pivot_longer(cols = 13:14, names_to = "scorer", values_to = "qual_stage") %>% mutate(scorer = gsub("Stage_","", scorer)) %>% filter(metric == "prop_egg") %>% mutate(pCO2 = factor(ifelse(pH == "low","elevated", "ambient"), levels = c("elevated","ambient"))) %>% filter(scorer == "Crandall")

#plot egg area x stage
jpeg("feature_area_by_staging_reg.jpg", width = 10, height = 4, units = "in", res = 300)

a <- quant_qual_long %>% ggplot(aes(x = as.numeric(qual_stage), y = um, group = interaction(sample_ID,scorer))) + geom_smooth(aes(group = Sex),method = "lm", color="black", fill = "gray80",size = 0.5)  + geom_point(aes(color = pCO2, shape = pCO2),size = 2)+ stat_poly_eq(aes(group = Sex,label = paste(after_stat(adj.rr.label), "*\", \"*", after_stat(p.value.label),sep = "")), label.x = "left", label.y = "top",formula = y~x, parse = TRUE, size = 2.8)+ labs(x = "stage", y = "eggs (% total area)", color =expression(paste(italic("p"),"CO", sep = "")[2]),shape =expression(paste(italic("p"),"CO", sep = "")[2])) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), axis.line = element_line(colour = "black")) + scale_y_continuous(limits =c(0,25)) + scale_x_continuous(n.breaks =3) + scale_shape_manual(values = c(17,16)) + theme(legend.key=element_rect(fill="white"))

#plot follicle area x stage
b <- quant_qual_long%>% ggplot(aes(x = as.numeric(qual_stage), y = um, group = interaction(sample_ID,scorer))) + geom_smooth(aes(group = Sex),method = "lm", color="black", fill = "gray80", size = 0.5)+ geom_point(aes(color = pCO2, shape = pCO2), size = 2)  + stat_poly_eq(aes(group = Sex,label = paste(after_stat(adj.rr.label), "*\", \"*", after_stat(p.value.label),sep = "")), label.x = "left", label.y = "top",formula = y~x, parse = TRUE, size = 2.8)  + labs(x = "stage", y = "follicles (% total area)", color =expression(paste(italic("p"),"CO", sep = "")[2]),shape =expression(paste(italic("p"),"CO", sep = "")[2])) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), axis.line = element_line(colour = "black"))  + scale_x_continuous(n.breaks =3)+theme(legend.key=element_rect(fill="white"))


#plot conx tissue area x stage
c <- quant_qual_long %>% ggplot(aes(x = as.numeric(qual_stage), y = um, group = interaction(sample_ID,scorer)))  + geom_smooth(aes(group = Sex),method = "lm", color="black", fill = "gray80", size =0.5)+ geom_point(aes(color = pCO2, shape = pCO2),size = 2) + stat_poly_eq(aes(group = Sex,label = paste(after_stat(adj.rr.label), "*\", \"*", after_stat(p.value.label),sep = "")), label.x = "left", label.y = "top",formula = y~x, parse = TRUE, size = 2.8) + labs(x = "stage", y = "connective tissue (% total area)", color =expression(paste(italic("p"),"CO", sep = "")[2]),shape =expression(paste(italic("p"),"CO", sep = "")[2])) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), axis.line = element_line(colour = "black")) + scale_x_continuous(n.breaks =3)+theme(legend.key=element_rect(fill="white"))

ggarrange(a,b,c, ncol = 3,labels = "auto", common.legend = T, legend = "bottom")
dev.off()
