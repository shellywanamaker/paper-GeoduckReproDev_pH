...
a <- quant_qual %>% pivot_longer(cols = 13:14, names_to = "scorer", values_to = "qual_stage") %>% mutate(scorer = gsub("Stage_","", scorer)) %>% filter(metric == "prop_egg") %>% mutate(pH = factor(pH, levels = c("low","ambient"))) %>% filter(scorer == "Crandall") %>% ggplot(aes(x = as.numeric(qual_stage), y = um, group = interaction(sample_ID,scorer))) + geom_smooth(aes(group = Sex),method = "lm", color="black", fill = "gray80",size = 0.5)  + geom_point(aes(color = pH, shape = pH),size = 2)+ stat_cor(aes(group = Sex), size = 3)  + labs(x = "stage", y = "eggs (% total area)") + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
...
b <- quant_qual %>% pivot_longer(cols = 13:14, names_to = "scorer", values_to = "qual_stage") %>% mutate(scorer = gsub("Stage_","", scorer)) %>% filter(metric == "prop_no_tissue") %>% mutate(pH = factor(pH, levels = c("low","ambient"))) %>% filter(scorer == "Crandall") %>% ggplot(aes(x = as.numeric(qual_stage), y = um, group = interaction(sample_ID,scorer))) + geom_smooth(aes(group = Sex),method = "lm", color="black", fill = "gray80", size = 0.5)+ geom_point(aes(color = pH, shape = pH), size = 2)  + stat_cor(aes(group = Sex), size = 3)  + labs(x = "stage", y = "follicles (% total area)") + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
...
c <- quant_qual %>% pivot_longer(cols = 13:14, names_to = "scorer", values_to = "qual_stage") %>% mutate(scorer = gsub("Stage_","", scorer)) %>% filter(metric == "prop_cnx_tissue_indve") %>% mutate(pH = factor(pH, levels = c("ambient","low"))) %>% filter(scorer == "Crandall") %>% ggplot(aes(x = as.numeric(qual_stage), y = um, group = interaction(sample_ID,scorer)))  + geom_smooth(aes(group = Sex),method = "lm", color="black", fill = "gray80", size =0.5)+ geom_point(aes(color = pH, shape = pH),size = 2) + stat_cor(aes(group = Sex), size = 3) + labs(x = "stage", y = "connective tissue (% total area)") + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), axis.line = element_line(colour = "black")) + scale_x_continuous(n.breaks =3)+theme(legend.key=element_rect(fill="white"))
