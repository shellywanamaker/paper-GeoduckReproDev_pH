...
```{r}
setwd("C:/Users/samjg/Documents/Github_Repositories/paper-GeoduckReproDev_pH/RAnalysis_male_histology/Data") #set working
...
Mean_Proportion_Table <- Male_hist_2 %>% 
  dplyr::select(c('Date', 'perc_zoa','perc_cytes', 'perc_lumen')) %>% 
  na.omit() %>%
  dplyr::group_by(Date) %>% 
...
```{r, mean percent SPERMATOZOA}
Mean_Proportion_Table$Date <- as.character(Mean_Proportion_Table$Date) # make Date a character variable!
# PERC ZOA TEST
PERCZoa_2factorialAOV <- aov(perc_zoa_mean ~ Treatment * Date, data = Mean_Proportion_Table)
leveneTest(perc_zoa_mean ~ Treatment * Date, data = Mean_Proportion_Table) # 0.3723 PASS
PERCZoa_aov_residuals <- residuals(object = PERCZoa_2factorialAOV) # Extract the residuals
shapiro.test(x = PERCZoa_aov_residuals) # Run Shapiro-Wilk test;  NORMAL 0.4989
...
# Treatment       1   16.2    16.2   0.248  0.624  
# Date            1  429.9   429.9   6.573  0.019 *
# Treatment:Date  1   34.7    34.7   0.531  0.475  
# Residuals      19 1242.6    65.4
TukeyHSD(PERCZoa_2factorialAOV, which = "Date") # 20190221-20190123 8.453121 - later date has 8.4 % more zoa


```

```{r, mean percent LUMEN}
Mean_Proportion_Table$Date <- as.character(Mean_Proportion_Table$Date) # make Date a character variable!
# PERC lumen TEST
PERClumen_2factorialAOV <- aov(perc_lumen_mean ~ Treatment * Date, data = Mean_Proportion_Table)
leveneTest(perc_lumen_mean ~ Treatment * Date, data = Mean_Proportion_Table) # 0.429 PASS
PERClumen_aov_residuals <- residuals(object = PERClumen_2factorialAOV) # Extract the residuals
shapiro.test(x = PERClumen_aov_residuals) # Run Shapiro-Wilk test;  NORMAL 0.1294
plot(PERClumen_2factorialAOV, 2)  # TEST ASSUMPTIONS    # 2. Normality - QQ plot (quantile quantile) of the model residuals
summary(PERClumen_2factorialAOV) # date is a sig effect in raw data (not transformed!)
#                Df Sum Sq Mean Sq F value Pr(>F)
# Treatment       1   15.5   15.46   0.859  0.366
# Date            1   18.2   18.21   1.011  0.327
# Treatment:Date  1   13.5   13.52   0.751  0.397
# Residuals      19  342.1   18.01 
```



```{r, mean percent SPERMATOCYTES}
Mean_Proportion_Table$Date <- as.character(Mean_Proportion_Table$Date) # make Date a character variable!
# PERC cytes TEST
PERCcytes_2factorialAOV <- aov(perc_cytes_mean ~ Treatment * Date, data = Mean_Proportion_Table)
leveneTest(perc_cytes_mean ~ Treatment * Date, data = Mean_Proportion_Table) # 0.5442 PASS
PERCcytes_aov_residuals <- residuals(object = PERCcytes_2factorialAOV) # Extract the residuals
shapiro.test(x = PERCcytes_aov_residuals) # Run Shapiro-Wilk test;  NORMAL 0.2247
plot(PERCcytes_2factorialAOV, 2)  # TEST ASSUMPTIONS    # 2. Normality - QQ plot (quantile quantile) of the model residuals
summary(PERCcytes_2factorialAOV) # date is a sig effect in raw data (not transformed!)
#                Df Sum Sq Mean Sq F value Pr(>F)
# Treatment       1   23.7   23.71   0.255  0.619
# Date            1  116.6  116.59   1.253  0.277
# Treatment:Date  1   84.6   84.63   0.910  0.352
# Residuals      19 1767.5   93.02 
```
